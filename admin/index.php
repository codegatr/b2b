...
register_shutdown_function(function() {
    ...
    if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR], true)) {
        ...
        echo '<strong style="font-size:15px">⚠️ PHP FATAL (admin)</strong><br>';
...
if ($page === 'invoice-pdf') {
    require B2B_ROOT . '/pages/invoice-pdf.php';
    exit;
}
...
$parasutMappingAjax = ($page === 'parasut-mapping' && ($_GET['ajax'] ?? '') === 'search');
if (!empty($_GET['export']) || !empty($_GET['print']) || $labelExport || $ajaxLookup || $batchAjax || $paymentsAjax || $priceListsAjax || $parasutMappingAjax) {
    // İzinli sayfaların tümünü buraya alın — örn. ledger PDF, raporlar PDF
    $allowedExportPages = ['ledger', 'reports', 'dealers', 'orders', 'stock-scan', 'payments', 'price-lists', 'parasut-mapping'];
...
$pageTitle = match($page) {
    ...
    'parasut-mapping' => 'Paraşüt Eşleme',
  ...
  <?php endif; ?>
    ...
    <nav class="sidebar-nav">
      ...
      <div class="nav-section">
        ...
        <a href="?page=manual-invoices" class="nav-item <?= $page==='manual-invoices'?'active':'' ?>">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="15" x2="15" y2="15"/><line x1="9" y1="11" x2="15" y2="11"/></svg>
          Manuel Faturalar
        </a>
      </div>
  ...
  <div class="main-content">
    <div class="topbar">
      ...
      <div class="topbar-actions" style="gap:6px">
        ...
        <a href="?page=logs" title="Log Kaydı"
           style="display:flex;align-items:center;gap:5px;padding:5px 10px;background:var(--bg);border:1px solid var(--border);border-radius:6px;font-size:12px;color:var(--text-2);text-decoration:none">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="10" y1="9" x2="8" y2="9"/></svg>
          Log Kaydı
        </a>
