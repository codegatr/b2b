...
// WAF (LiteSpeed/Imunify360) 6 haneli rakam içeren parametreleri kart no
// pattern olarak algılayıp 403 atıyor. JS bin'i base64 encode olarak
// 'x' parametresinde yolluyor — body'de saf rakam kalmıyor.
$rawX = $_POST['x'] ?? '';
$bin  = '';
if ($rawX !== '') {
    $decoded = base64_decode($rawX, true);
    if ($decoded !== false) {
        $bin = preg_replace('/\D/', '', $decoded);
    }
}
// Backwards compat — eski parametreleri de destekle
if ($bin === '') {
    $bin = preg_replace('/\D/', '',
        ($_POST['bp1'] ?? $_POST['bin'] ?? '') .
        ($_POST['bp2'] ?? '')
