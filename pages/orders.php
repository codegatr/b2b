...
register_shutdown_function(function() {
    ...
    if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR], true)) {
        ...
        echo '<strong style="font-size:15px">⚠️ PHP FATAL (orders.php)</strong><br>';
...
<div class="tab-bar mb-4">
    <?php foreach ([''=> 'Tümü','bekliyor'=>'Bekleyen','onaylandi'=>'Onaylanan','kargoda'=>'Teslimata Çıktı','teslim_edildi'=>'Teslim','iptal'=>'İptal'] as $v=>$l): ?>
...
if ($payable && ($order['payment_method'] ?? '') === 'havale_eft' && $bankHtml !== ''):
?>
<div class="card" style="margin-bottom:16px;border-left:4px solid var(--primary)">
  <div class="card-body">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
      ...
      <div style="flex:1">
        <h3 style="margin:0 0 2px;font-size:15px;font-weight:700">Banka Hesap Bilgileri</h3>
        <p style="margin:0;font-size:12px;color:var(--text-muted)">
          Sipariş tutarı: <strong style="color:var(--red)"><?= money((float)$order['grand_total']) ?></strong>
          — Ödeme yaptıktan sonra <strong>Havale Bildir</strong> butonu ile dekontunuzu yükleyin.
        </p>
      </div>
    </div>
    <?= $bankHtml ?>
...
$labels = ['Sipariş Alındı','Onaylandı','Hazırlanıyor','Teslimata Çıktı','Teslim Edildi'];
$icons  = ['🕐','✅','📦','🚚','🏠'];
